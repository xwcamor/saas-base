...
sudo chown -R deploy:www-data /var/www/blog_main_base

# Permisos generales: directorios 755, archivos 644
sudo find /var/www/blog_main_base -type d -exec chmod 755 {} \;
sudo find /var/www/blog_main_base -type f -exec chmod 644 {} \;

# storage/ y bootstrap/cache/ necesitan escritura por PHP-FPM
sudo chmod -R 775 /var/www/blog_main_base/storage
sudo chmod -R 775 /var/www/blog_main_base/bootstrap/cache
```
...
php artisan tinker
>>> $u = User::where('email', 'carlosmorales@gmail.com')->first();
...
server {
    ...
    root /var/www/blog_main_base/public;
    index index.php;
...
command=php /var/www/blog_main_base/artisan queue:work --queue=default --sleep=3 --tries=3 --max-time=3600
autostart=true
autorestart=true
user=deploy
numprocs=2
...
```bash
tail -f /var/www/blog_main_base/storage/logs/laravel.log
```
...
# Disco
df -h
du -sh /var/www/blog_main_base/storage/   # ¿está creciendo?
...
1. `tail -100 /var/www/blog_main_base/storage/logs/laravel.log` — leer el último error
...
- [ ] Revisar uso de disco (`df -h`, `du -sh /var/www/blog_main_base/storage/`)
