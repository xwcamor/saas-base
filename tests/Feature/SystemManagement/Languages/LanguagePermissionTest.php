 ...
 * be able to read, create, edit, delete or export languages. Workers/users
 ...
 */
class LanguagePermissionTest extends LanguageTestCase
{
    ...
    public function test_admin_cannot_create_languages(): void
    {
        $this->actingAsAdmin();
        $response = $this->post(route('system_management.languages.store'), ['name' => 'X', 'iso_code' => 'xy']);
        $response->assertRedirect();
        $this->assertSame(0, Language::count(), 'No language should have been created.');
    }

    public function test_admin_cannot_export(): void
    {
        $this->actingAsAdmin();
        $response = $this->post(route('system_management.languages.export_excel'), [
            'columns' => ['id', 'name'],
        ]);
        $response->assertRedirect();
        $response->assertSessionHas('error');
    ...
    {
        $this->actingAsAdmin();
        $response = $this->post(route('system_management.languages.import'));
        $response->assertRedirect();
        $response->assertSessionHas('error');
    }
