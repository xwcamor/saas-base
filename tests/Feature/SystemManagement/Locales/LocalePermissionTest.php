...
/**
 * Verifies route protection for the Locales module.
 *
 * Locales is a master table → super ONLY. Even tenant admins must NOT
 * be able to read, create, edit, delete or export locales. Workers/users
 ...
 */
class LocalePermissionTest extends LocaleTestCase
{
    ...
    public function test_admin_cannot_create_locales(): void
    {
        $this->actingAsAdmin();
        $response = $this->post(route('system_management.locales.store'), ['name' => 'X']);
        $response->assertRedirect();
        $this->assertSame(0, Locale::count(), 'No locale should have been created.');
    }

    public function test_admin_cannot_export(): void
    {
        $this->actingAsAdmin();
        $response = $this->post(route('system_management.locales.export_excel'), [
            'columns' => ['id', 'name'],
        ]);
        $response->assertRedirect();
        $response->assertSessionHas('error');
    ...
    {
        $this->actingAsAdmin();
        $response = $this->post(route('system_management.locales.import'));
        $response->assertRedirect();
        $response->assertSessionHas('error');
    }
