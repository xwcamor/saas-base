...
<template>
    <Drawer
        :open="open"
        :title="$t('locales.singular') + ' — ' + $t('global.show')"
        :width="width"
        placement="right"
    ...
    >
        <template v-if="locale">
            ...
            <Descriptions :column="1" bordered class="mt-4">
                ...
                <DescriptionsItem v-if="locale.creator" :label="$t('global.created_by')">
                    {{ locale.creator.name }}
                    <span class="audit-email">({{ locale.creator.email }})</span>
                </DescriptionsItem>
        ...
        <template #footer>
            <div v-if="locale" class="drawer-footer" :class="{ 'drawer-footer--mobile': isMobile }">
                <Link
                    v-if="canDelete"
                    :href="route('system_management.locales.delete', locale.slug)"
                >
                ...
                <Button
                    v-if="canCreate"
                    :block="isMobile"
                    :loading="duplicatingId === locale.id"
                    @click="emit('duplicate', locale)"
                >
                    <CopyOutlined /> {{ $t('global.duplicate') }}
                </Button>
                <Link
                    v-if="canEdit"
                    :href="route('system_management.locales.edit', locale.slug)"
                >
