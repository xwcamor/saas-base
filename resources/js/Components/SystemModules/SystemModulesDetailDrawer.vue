...
const props = defineProps({
    ...
    system_module:      { type: Object,  default: null },
...
<template>
    <Drawer
        :open="open"
        :title="$t('system_modules.singular') + ' — ' + $t('global.show')"
        :width="width"
        placement="right"
        @update:open="emit('update:open', $event)"
    >
        <template v-if="system_module">
            <div class="drawer-hero">
                <div class="drawer-hero__icon">
                    <GlobalOutlined />
                </div>
                <div>
                    <h2>{{ system_module.name }}</h2>
            ...
            <Descriptions :column="1" bordered class="mt-4">
                ...
                <DescriptionsItem v-if="system_module.creator" :label="$t('global.created_by')">
                    ...
                    <span class="audit-email">({{ system_module.creator.email }})</span>
                </DescriptionsItem>
                <DescriptionsItem :label="$t('global.updated_at')">
                    {{ formatDateTime(system_module.updated_at) }}
                </DescriptionsItem>
            </Descriptions>
        </template>
        ...
        <template #footer>
            <div v-if="system_module" class="drawer-footer" :class="{ 'drawer-footer--mobile': isMobile }">
                <Link
                    v-if="canDelete"
                    :href="route('system_management.system_modules.delete', system_module.slug)"
                >
                ...
                <Button
                    v-if="canCreate"
                    :block="isMobile"
                    :loading="duplicatingId === system_module.id"
                    @click="emit('duplicate', system_module)"
                >
                    <CopyOutlined /> {{ $t('global.duplicate') }}
                </Button>
                <Link
                    v-if="canEdit"
                    :href="route('system_management.system_modules.edit', system_module.slug)"
                >
