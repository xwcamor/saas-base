...
<template>
    <Drawer
        :open="open"
        :title="$t('regions.singular') + ' — ' + $t('global.show')"
        :width="width"
        placement="right"
    ...
    >
        <template v-if="region">
            ...
            <Descriptions :column="1" bordered class="mt-4">
                ...
                <DescriptionsItem v-if="region.creator" :label="$t('global.created_by')">
                    {{ region.creator.name }}
                    <span class="audit-email">({{ region.creator.email }})</span>
                </DescriptionsItem>
        ...
        <template #footer>
            <div v-if="region" class="drawer-footer" :class="{ 'drawer-footer--mobile': isMobile }">
                <Link
                    v-if="canDelete"
                    :href="route('system_management.regions.delete', region.slug)"
                >
                ...
                <Button
                    v-if="canCreate"
                    :block="isMobile"
                    :loading="duplicatingId === region.id"
                    @click="emit('duplicate', region)"
                >
                    <CopyOutlined /> {{ $t('global.duplicate') }}
                </Button>
                <Link
                    v-if="canEdit"
                    :href="route('system_management.regions.edit', region.slug)"
                >
