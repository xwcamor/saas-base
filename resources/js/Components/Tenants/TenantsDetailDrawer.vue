...
<template>
    <Drawer
        :open="open"
        :title="$t('tenants.singular') + ' — ' + $t('global.show')"
        :width="width"
        placement="right"
    ...
    >
        <template v-if="tenant">
            ...
            <Descriptions :column="1" bordered class="mt-4">
                ...
                <DescriptionsItem v-if="tenant.creator" :label="$t('global.created_by')">
                    {{ tenant.creator.name }}
                    <span class="audit-email">({{ tenant.creator.email }})</span>
                </DescriptionsItem>
        ...
        <template #footer>
            <div v-if="tenant" class="drawer-footer" :class="{ 'drawer-footer--mobile': isMobile }">
                <Link
                    v-if="canDelete"
                    :href="route('system_management.tenants.delete', tenant.slug)"
                >
                ...
                <Button
                    v-if="canCreate"
                    :block="isMobile"
                    :loading="duplicatingId === tenant.id"
                    @click="emit('duplicate', tenant)"
                >
                    <CopyOutlined /> {{ $t('global.duplicate') }}
                </Button>
                <Link
                    v-if="canEdit"
                    :href="route('system_management.tenants.edit', tenant.slug)"
                >
