<script setup>
/**
 * Tabla editable in-line del flujo Edit-All de Users. Recibe `draft` por
 * v-model y predicados isDirty/isDuplicate del composable useEditAllDraft.
 * El email se muestra read-only (es unico y se cambia uno por uno).
 */
    ...
    <table v-if="draft.length > 0" class="edit-table">
        <thead>
            <tr>
                ...
                <th class="col-name">{{ $t('users.table_headers.editable_name') }}</th>
                <th class="col-email">{{ $t('users.email') }}</th>
                <th class="col-status">{{ $t('users.table_headers.editable_status') }}</th>
            </tr>
        </thead>
        <tbody>
            <tr
            ...
            >
                ...
                <td class="col-email">
                    <span class="email-readonly">{{ row.email }}</span>
                </td>
                <td class="col-status">
                    <Switch
...
@media (max-width: 768px) {
    .edit-table .col-id, .edit-table .col-email { display: none; }
