...
import TenantsTrashBulkBar from '@/Components/Tenants/TenantsTrashBulkBar.vue';
import TenantsForceDeleteModal from '@/Components/Tenants/TenantsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { tenantsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('system_management.tenants.index'));
}
...
const {
    ...
    restoreRouteName:     'system_management.tenants.restore',
    bulkRestoreRouteName: 'system_management.tenants.bulk_restore',
});
...
const openForceDelete = (tenant) => {
    forceDeleteTarget.value = tenant;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('system_management.tenants.force_delete', forceDeleteTarget.value.slug),
        {
...
// ─── Columns + paginación ────────────────────────────────────────────────
const columns = computed(() => tenantsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.tenants.total === 1 ? t('global.record') : t('global.records');
    return `${props.tenants.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('tenants.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('system_management.tenants.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('tenants.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('tenants.restore_hint')"
