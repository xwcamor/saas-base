...
const submit = () => {
    form.delete(route('system_management.tenants.deleteSave', props.tenant.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('tenants.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('system_management.tenants.index')"
            :title="$t('global.delete') + ' ' + $t('tenants.record')"
            :subtitle="$t('tenants.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('tenants.delete_about', { name: tenant.name }) }}
                </template>
            ...
            <!-- Dependency check warning — solo aparece si hay registros que
                 dependen de esta región (ej. countries.tenant_id). El backend
            ...
            <div class="tenant-summary">
                <div class="tenant-summary__row">
                    <span class="tenant-summary__label">ID</span>
                    <span class="tenant-summary__value">{{ tenant.id }}</span>
                </div>
                <div class="tenant-summary__row">
                    <span class="tenant-summary__label">Slug</span>
                    <span class="tenant-summary__value"><code>{{ tenant.slug }}</code></span>
                </div>
                <div class="tenant-summary__row">
                    <span class="tenant-summary__label">{{ $t('tenants.name') }}</span>
                    <span class="tenant-summary__value">{{ tenant.name }}</span>
                </div>
                <div class="tenant-summary__row">
                    <span class="tenant-summary__label">{{ $t('tenants.is_active') }}</span>
                    <span class="tenant-summary__value">
                        <Tag :color="tenant.is_active ? 'success' : 'error'" :bordered="false">
