...
<template>
    ...
    <div class="edit-all">
        ...
        >
            ...
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('roles.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('roles.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('roles.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <div v-if="roles.total > roles.per_page" class="pagination">
            <Pagination
