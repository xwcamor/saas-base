...
const submit = () => {
    form.delete(route('system_management.locales.deleteSave', props.locale.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('locales.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('system_management.locales.index')"
            :title="$t('global.delete') + ' ' + $t('locales.record')"
            :subtitle="$t('locales.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('locales.delete_about', { name: locale.name }) }}
                </template>
            ...
            <!-- Dependency check warning — solo aparece si hay registros que
                 dependen de esta región (ej. countries.locale_id). El backend
            ...
            <div class="locale-summary">
                <div class="locale-summary__row">
                    <span class="locale-summary__label">ID</span>
                    <span class="locale-summary__value">{{ locale.id }}</span>
                </div>
                <div class="locale-summary__row">
                    <span class="locale-summary__label">Slug</span>
                    <span class="locale-summary__value"><code>{{ locale.slug }}</code></span>
                </div>
                <div class="locale-summary__row">
                    <span class="locale-summary__label">{{ $t('locales.name') }}</span>
                    <span class="locale-summary__value">{{ locale.name }}</span>
                </div>
                <div class="locale-summary__row">
                    <span class="locale-summary__label">{{ $t('locales.is_active') }}</span>
                    <span class="locale-summary__value">
                        <Tag :color="locale.is_active ? 'success' : 'error'" :bordered="false">
...
.locale-summary__label {
    color: var(--color-text-muted);
    font-weight: 500;
}
.locale-summary__value {
