...
import LocalesEditAllTable from '@/Components/Locales/LocalesEditAllTable.vue';
...
const source = computed(() => props.locales.data);
const { draft, isDirty, dirtyCount, dirtyChanges, duplicateRows, discardAll } = useEditAllDraft({
    source,
    editableFields: ['name', 'code', 'language_id', 'is_active'],
    uniqueField:    'name',
});
...
const saveAll = () => {
    ...
    router.post(
        route('system_management.locales.edit_all.update'),
...
<template>
    <Head :title="$t('locales.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('system_management.locales.index')"
            :title="$t('global.edit_all') + ' — ' + $t('locales.plural')"
            :subtitle="$t('locales.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('locales.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('locales.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('locales.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <div v-if="locales.total > locales.per_page" class="pagination">
            <Pagination
                :current="locales.current_page"
                :pageSize="locales.per_page"
                :total="locales.total"
