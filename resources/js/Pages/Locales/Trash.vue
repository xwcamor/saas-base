...
import LocalesTrashBulkBar from '@/Components/Locales/LocalesTrashBulkBar.vue';
import LocalesForceDeleteModal from '@/Components/Locales/LocalesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { localesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('system_management.locales.index'));
}
...
const {
    ...
    restoreRouteName:     'system_management.locales.restore',
    bulkRestoreRouteName: 'system_management.locales.bulk_restore',
});
...
const openForceDelete = (locale) => {
    forceDeleteTarget.value = locale;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('system_management.locales.force_delete', forceDeleteTarget.value.slug),
        {
...
// ─── Columns + paginación ────────────────────────────────────────────────
const columns = computed(() => localesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.locales.total === 1 ? t('global.record') : t('global.records');
    return `${props.locales.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('locales.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('system_management.locales.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('locales.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('locales.restore_hint')"
            ...
            <Empty
                v-if="locales.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <LocalesForceDeleteModal
