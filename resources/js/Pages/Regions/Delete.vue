...
const submit = () => {
    form.delete(route('system_management.regions.deleteSave', props.region.slug), {
        preserveScroll: true,
    });
};
</script>
...
<template>
    <Head :title="$t('global.delete') + ' — ' + $t('regions.singular')" />

    <div class="delete-page">
        <SectionHeader
            :back-href="route('system_management.regions.index')"
            :title="$t('global.delete') + ' ' + $t('regions.record')"
            :subtitle="$t('regions.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('regions.delete_about', { name: region.name }) }}
                </template>
            ...
            <!-- Dependency check warning — solo aparece si hay registros que
                 dependen de esta región (ej. countries.region_id). El backend
            ...
            <div class="region-summary">
                <div class="region-summary__row">
                    <span class="region-summary__label">ID</span>
                    <span class="region-summary__value">{{ region.id }}</span>
                </div>
                <div class="region-summary__row">
                    <span class="region-summary__label">Slug</span>
                    <span class="region-summary__value"><code>{{ region.slug }}</code></span>
                </div>
                <div class="region-summary__row">
                    <span class="region-summary__label">{{ $t('regions.name') }}</span>
                    <span class="region-summary__value">{{ region.name }}</span>
                </div>
                <div class="region-summary__row">
                    <span class="region-summary__label">{{ $t('regions.is_active') }}</span>
                    <span class="region-summary__value">
                        <Tag :color="region.is_active ? 'success' : 'error'" :bordered="false">
