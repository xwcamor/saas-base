...
import RegionsEditAllTable from '@/Components/Regions/RegionsEditAllTable.vue';
...
const source = computed(() => props.regions.data);
...
const saveAll = () => {
    ...
    router.post(
        route('system_management.regions.edit_all.update'),
...
<template>
    <Head :title="$t('regions.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('system_management.regions.index')"
            :title="$t('global.edit_all') + ' — ' + $t('regions.plural')"
            :subtitle="$t('regions.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('regions.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('regions.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('regions.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <div v-if="regions.total > regions.per_page" class="pagination">
            <Pagination
                :current="regions.current_page"
                :pageSize="regions.per_page"
                :total="regions.total"
