...
import SystemModulesTrashBulkBar from '@/Components/SystemModules/SystemModulesTrashBulkBar.vue';
import SystemModulesForceDeleteModal from '@/Components/SystemModules/SystemModulesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { system_modulesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('system_management.system_modules.index'));
}
...
const {
    ...
    restoreRouteName:     'system_management.system_modules.restore',
    bulkRestoreRouteName: 'system_management.system_modules.bulk_restore',
});
...
const openForceDelete = (system_module) => {
    forceDeleteTarget.value = system_module;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('system_management.system_modules.force_delete', forceDeleteTarget.value.slug),
        {
...
// ─── Columns + paginación ────────────────────────────────────────────────
const columns = computed(() => system_modulesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.system_modules.total === 1 ? t('global.record') : t('global.records');
    return `${props.system_modules.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('system_modules.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('system_management.system_modules.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('system_modules.plural')"
            :subtitle="subtitle"
        ...
        <Card :bodyStyle="{ padding: 0 }" class="grid-card">
            ...
            <ResponsiveTable
                :dataSource="system_modules.data"
                :columns="columns"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('system_modules.restore_hint')"
            ...
            <Empty
                v-if="system_modules.data.length === 0"
                :description="$t('global.no_deleted_records')"
                style="padding: 48px 16px"
            />
        </Card>

        <SystemModulesForceDeleteModal
