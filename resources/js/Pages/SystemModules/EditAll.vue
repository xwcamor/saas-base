...
import SystemModulesEditAllTable from '@/Components/SystemModules/SystemModulesEditAllTable.vue';
...
const source = computed(() => props.system_modules.data);
...
const saveAll = () => {
    ...
    router.post(
        route('system_management.system_modules.edit_all.update'),
...
<template>
    <Head :title="$t('system_modules.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('system_management.system_modules.index')"
            :title="$t('global.edit_all') + ' — ' + $t('system_modules.plural')"
            :subtitle="$t('system_modules.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('system_modules.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('system_modules.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('system_modules.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('system_modules.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <SystemModulesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="system_modules.total > system_modules.per_page" class="pagination">
            <Pagination
                :current="system_modules.current_page"
                :pageSize="system_modules.per_page"
                :total="system_modules.total"
