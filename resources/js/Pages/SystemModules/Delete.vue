...
<template>
    ...
    <div class="delete-page">
        <SectionHeader
            ...
            :subtitle="$t('system_modules.delete_hint')"
        ...
        <Card class="delete-card" :bodyStyle="{ padding: '24px 28px' }">
            ...
            >
                <template #message>
                    {{ $t('system_modules.delete_about', { name: system_module.name }) }}
                </template>
            ...
            <!-- Dependency check warning — solo aparece si hay registros que
                 dependen de esta región (ej. countries.system_module_id). El backend
            ...
            <div class="system_module-summary">
                <div class="system_module-summary__row">
                    <span class="system_module-summary__label">ID</span>
                    <span class="system_module-summary__value">{{ system_module.id }}</span>
                </div>
                <div class="system_module-summary__row">
                    <span class="system_module-summary__label">Slug</span>
                    <span class="system_module-summary__value"><code>{{ system_module.slug }}</code></span>
                </div>
                <div class="system_module-summary__row">
                    <span class="system_module-summary__label">{{ $t('system_modules.name') }}</span>
                    <span class="system_module-summary__value">{{ system_module.name }}</span>
                </div>
                <div class="system_module-summary__row">
                    <span class="system_module-summary__label">{{ $t('system_modules.is_active') }}</span>
                    <span class="system_module-summary__value">
...
.system_module-summary__label {
    color: var(--color-text-muted);
    font-weight: 500;
}
.system_module-summary__value {
