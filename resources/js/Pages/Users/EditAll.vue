...
import UsersEditAllTable from '@/Components/Users/UsersEditAllTable.vue';
...
const source = computed(() => props.users.data);
...
const saveAll = () => {
    ...
    router.post(
        route('user_management.users.edit_all.update'),
...
<template>
    <Head :title="$t('users.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('user_management.users.index')"
            :title="$t('global.edit_all') + ' — ' + $t('users.plural')"
            :subtitle="$t('users.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('users.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('users.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('users.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <div v-if="users.total > users.per_page" class="pagination">
            <Pagination
