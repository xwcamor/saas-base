...
import LanguagesTrashBulkBar from '@/Components/Languages/LanguagesTrashBulkBar.vue';
import LanguagesForceDeleteModal from '@/Components/Languages/LanguagesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { languagesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('system_management.languages.index'));
}
...
const {
    ...
    restoreRouteName:     'system_management.languages.restore',
    bulkRestoreRouteName: 'system_management.languages.bulk_restore',
});
...
const openForceDelete = (language) => {
    forceDeleteTarget.value = language;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('system_management.languages.force_delete', forceDeleteTarget.value.slug),
        {
...
// ─── Columns + paginación ────────────────────────────────────────────────
const columns = computed(() => languagesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.languages.total === 1 ? t('global.record') : t('global.records');
    return `${props.languages.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('languages.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('system_management.languages.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('languages.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('languages.restore_hint')"
