...
import LanguagesEditAllTable from '@/Components/Languages/LanguagesEditAllTable.vue';
...
const saveAll = () => {
    ...
    router.post(
        route('system_management.languages.edit_all.update'),
...
<template>
    <Head :title="$t('languages.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('system_management.languages.index')"
            :title="$t('global.edit_all') + ' — ' + $t('languages.plural')"
            :subtitle="$t('languages.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('languages.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('languages.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('languages.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('languages.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <LanguagesEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="languages.total > languages.per_page" class="pagination">
            <Pagination
                :current="languages.current_page"
