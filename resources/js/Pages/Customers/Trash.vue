...
import CustomersTrashBulkBar from '@/Components/Customers/CustomersTrashBulkBar.vue';
import CustomersForceDeleteModal from '@/Components/Customers/CustomersForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { customersTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('business_management.customers.index'));
}
...
const {
    ...
    restoreRouteName:     'business_management.customers.restore',
    bulkRestoreRouteName: 'business_management.customers.bulk_restore',
});
...
const openForceDelete = (customer) => {
    forceDeleteTarget.value = customer;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('business_management.customers.force_delete', forceDeleteTarget.value.slug),
        {
...
const columns = computed(() => customersTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.customers.total === 1 ? t('global.record') : t('global.records');
    return `${props.customers.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('customers.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('business_management.customers.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('customers.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('customers.restore_hint')"
