...
import CountriesTrashBulkBar from '@/Components/Countries/CountriesTrashBulkBar.vue';
import CountriesForceDeleteModal from '@/Components/Countries/CountriesForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { countriesTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('system_management.countries.index'));
}
...
const {
    ...
    restoreRouteName:     'system_management.countries.restore',
    bulkRestoreRouteName: 'system_management.countries.bulk_restore',
});
...
const openForceDelete = (country) => {
    forceDeleteTarget.value = country;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('system_management.countries.force_delete', forceDeleteTarget.value.slug),
        {
...
// ─── Columns + paginación ────────────────────────────────────────────────
const columns = computed(() => countriesTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.countries.total === 1 ? t('global.record') : t('global.records');
    return `${props.countries.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('countries.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('system_management.countries.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('countries.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('countries.restore_hint')"
