...
import SettingsTrashBulkBar from '@/Components/Settings/SettingsTrashBulkBar.vue';
import SettingsForceDeleteModal from '@/Components/Settings/SettingsForceDeleteModal.vue';

import { useModuleRestore } from '@/Composables/useModuleRestore';
import { useDateFormat } from '@/Composables/useDateFormat';
import { useI18n } from '@/Plugins/i18n';
import { settingsTrashColumns } from './config/trashColumns';
...
if (!isSuper) {
    router.visit(route('system_management.settings.index'));
}
...
const {
    ...
    restoreRouteName:     'system_management.settings.restore',
    bulkRestoreRouteName: 'system_management.settings.bulk_restore',
});
...
const openForceDelete = (setting) => {
    forceDeleteTarget.value = setting;
...
const submitForceDelete = () => {
    ...
    forceDeleteErrors.value = {};
    router.delete(
        route('system_management.settings.force_delete', forceDeleteTarget.value.slug),
        {
...
// ─── Columns + paginación ────────────────────────────────────────────────
const columns = computed(() => settingsTrashColumns(t));
...
const subtitle = computed(() => {
    const word = props.settings.total === 1 ? t('global.record') : t('global.records');
    return `${props.settings.total} ${word} · ${t('global.super_only')}`;
});
</script>

<template>
    <Head :title="$t('global.view_deleted') + ' — ' + $t('settings.plural')" />

    <div v-if="isSuper">
        <SectionHeader
            :back-href="route('system_management.settings.index')"
            :title="$t('global.view_deleted') + ' — ' + $t('settings.plural')"
            :subtitle="subtitle"
                    ...
                    <template v-else-if="column.key === 'actions'">
                        <Space :size="4">
                            <Popconfirm
                                ...
                                :description="$t('settings.restore_hint')"
