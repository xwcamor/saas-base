...
import SettingsEditAllTable from '@/Components/Settings/SettingsEditAllTable.vue';
...
const source = computed(() => props.settings.data);
...
const saveAll = () => {
    ...
    router.post(
        route('system_management.settings.edit_all.update'),
...
<template>
    <Head :title="$t('settings.edit_all_title')" />

    <div class="edit-all">
        <SectionHeader
            :back-href="route('system_management.settings.index')"
            :title="$t('global.edit_all') + ' — ' + $t('settings.plural')"
            :subtitle="$t('settings.edit_all_subtitle')"
        >
            <template #icon><EditOutlined /></template>
            <template #actions>
                <Button :disabled="dirtyCount === 0 || submitting" @click="discardAll">
                    <UndoOutlined /> {{ $t('settings.edit_all_discard') }}
                </Button>
                <Button
                    type="primary"
                    :loading="submitting"
                    ...
                    @click="saveAll"
                >
                    <SaveOutlined /> {{ $t('settings.edit_all_save_all') }}
                </Button>
            </template>
        </SectionHeader>
        ...
        <Alert v-if="dirtyCount > 0" type="info" show-icon class="status-bar">
            <template #message>
                {{ $t('settings.edit_all_changes', { count: dirtyCount }) }}
            </template>
        </Alert>
        ...
        <Alert
            ...
            type="error"
            show-icon
            :message="$t('settings.name_unique')"
            class="status-bar"
        />

        <Card :bodyStyle="{ padding: 0 }" class="edit-table-card">
            <SettingsEditAllTable
                v-model:draft="draft"
                :is-dirty="isDirty"
        ...
        <div v-if="settings.total > settings.per_page" class="pagination">
            <Pagination
                :current="settings.current_page"
