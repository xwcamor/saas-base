...
<head>
    ...
    <style>
        /*
         | Reglas para PDFs en DomPDF (clon del template de Regions):
         |
                ...
                @foreach ($customers as $customer)
                    <tr>
                        @foreach ($columns as $col)
                            <td>
                                @switch($col)
                                    @case('id')        {{ $customer->id }} @break
                                    @case('name')      {{ $customer->name }} @break
                                    @case('cod')       {{ $customer->cod ?? '' }} @break
                                    @case('country')   {{ $customer->country?->name ?? '—' }} @break
                                    @case('is_active')
                                        <span class="{{ $customer->is_active ? 'status-active' : 'status-inactive' }}">
                                            {{ $customer->state_text }}
                                        </span>
                                    @break
                                    @case('slug')       {{ $customer->slug }} @break
                                    @case('created_at') {{ $customer->created_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('updated_at') {{ $customer->updated_at?->copy()->setTimezone($tz ?? config('app.timezone'))->format(\App\Support\Tz::DATETIME_FORMAT) }} @break
                                    @case('creator')    {{ $customer->creator->name ?? '—' }} @break
