...
class Customer extends Model
{
    ...
    protected $fillable = [
        'slug', 'name', 'cod', 'country_id', 'is_active', 'tenant_id',
    ...
    public function scopeFilter($query, $request)
    {
        ...
        $query->when($request->filled('name'), function ($q) use ($request, $isPgsql, $tbl) {
            ...
            $q->where(function ($qq) use ($names, $isPgsql, $tbl) {
                foreach ($names as $name) {
                    ...
                    if ($isPgsql) {
                        $qq->orWhereRaw("unaccent(lower({$tbl}.name)) LIKE unaccent(lower(?)) ESCAPE '\\'", [$needle]);
                    } else {
        ...
        $query->when($request->filled('cod'), function ($q) use ($request, $tbl) {
            $q->whereRaw("{$tbl}.cod LIKE ? ESCAPE '\\\\'", [LikeQuery::contains((string) $request->cod)]);
        });
