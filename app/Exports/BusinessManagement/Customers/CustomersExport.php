 ...
 */
class CustomersExport implements FromCollection, WithEvents, WithTitle
{
    protected $customers;
    protected int $count;
    ...
    public function __construct($customers, array $options = [], ?int $count = null)
    {
        ...
        $this->customers = is_array($customers) ? collect($customers) : $customers;
        $this->options   = $options;
        $this->count     = $count !== null
            ? $count
            : (is_countable($customers) ? count($customers) : iterator_count($customers));
        ...
        $tz = $this->tz;
        $this->columnDefs = [
            'id'         => ['heading' => __('customers.id'),        'value' => fn($c, $i) => $c->id],
            'name'       => ['heading' => __('customers.name'),      'value' => fn($c, $i) => $c->name],
            'cod'        => ['heading' => __('customers.cod'),       'value' => fn($c, $i) => $c->cod ?? ''],
            'country'    => ['heading' => __('customers.country'),   'value' => fn($c, $i) => $c->country?->name ?? '—'],
            'is_active'  => ['heading' => __('customers.is_active'), 'value' => fn($c, $i) => $c->state_text],
    ...
    {
        $title    = $this->options['title'] ?? __('customers.export_title');
        $count    = $this->count;
        $subtitle = sprintf(
            '%s · %s · %s',
        ...
        foreach ($this->customers as $customer) {
            $rows->push(array_map(
                fn($k) => $this->columnDefs[$k]['value']($customer, $i),
                $this->activeColumns
            ));
            $i++;
        }
    ...
    {
        return mb_substr($this->options['title'] ?? __('customers.export_title'), 0, 31);
    }
