...
/**
 * Generates a styled .docx report of Customers using PhpWord directly
...
class CustomersWord
{
    /** Map column key → ['heading' => string, 'value' => fn($customer) => mixed] */
    ...
    public function generate(
        ...
        $this->columnDefs = [
            'id'         => ['heading' => __('customers.id'),        'value' => fn($c) => (string) $c->id],
            'name'       => ['heading' => __('customers.name'),      'value' => fn($c) => (string) $c->name],
            'cod'        => ['heading' => __('customers.cod'),       'value' => fn($c) => (string) ($c->cod ?? '')],
            'country'    => ['heading' => __('customers.country'),   'value' => fn($c) => (string) ($c->country?->name ?? '—')],
            'is_active'  => ['heading' => __('customers.is_active'), 'value' => fn($c) => $c->state_text],
        ...
        $title         = $options['title']         ?? __('customers.export_title');
        ...
        if ($count === 0) {
            ...
            $phpWord->addTableStyle('CustomersTable', [
                'borderSize'      => 4,
            ...
            $table = $section->addTable('CustomersTable');
            ...
            foreach ($customers as $i => $customer) {
                ...
                foreach ($columns as $col) {
                    ...
                    $value = $this->columnDefs[$col]['value']($customer);

                    if ($col === 'is_active') {
                        $color = $customer->is_active ? '1D7044' : 'C8281D';
