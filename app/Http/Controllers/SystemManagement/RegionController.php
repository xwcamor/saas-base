...
use App\Http\Requests\SystemManagement\Region\BulkDeleteRequest;
use App\Http\Requests\SystemManagement\Region\BulkSetActiveRequest;
use App\Http\Requests\SystemManagement\Region\BulkRestoreRequest;
use App\Http\Requests\SystemManagement\Region\ForceDeleteRequest;
use App\Http\Requests\SystemManagement\Region\EditAllUpdateRequest;
use App\Http\Requests\SystemManagement\Region\ImportRequest;
use App\Services\SystemManagement\RegionService;
use Illuminate\Http\Request;

/**
 * Permisos: todas las rutas viven dentro de `role:super` middleware en
 * routes/system_management.php. Por eso NO chequeamos permission:regions.*
...
class RegionController extends Controller
{
    public function index(Request $request)
    {
        ...
        $regions = Region::filter($request)
            ->select('regions.id', 'regions.slug', 'regions.name', 'regions.is_active', 'regions.created_at', 'regions.updated_at', 'regions.created_by')
        ...
        $regions->getCollection()->transform(function ($r) {
            $r->is_favorite = (bool) ($r->is_favorite ?? false);
            return $r;
        });

        $totalUnfiltered = Region::count();
    ...
    public function show(Request $request, $slug)
    {
        ...
        $query = Region::with(['creator:id,name,email', 'deleter:id,name,email']);
        if ($isSuper) {
            $query->withTrashed();
        }

        $region = $query->where('slug', $slug)->firstOrFail();

        // Track recent view (best-effort, no rompe la pantalla si falla).
        if ($userId = $request->user()?->id) {
            try {
                \App\Models\UserRecentView::track($userId, Region::class, $region->id);
            } catch (\Throwable $e) {
                // silent fail
            }
        }
        ...
        $activity = $canSeeAudit
            ? AuditLogResource::collection(
                \App\Models\AuditLog::query()
                    ->where('auditable_type', \App\Models\Region::class)
        ...
        return inertia('Regions/Show', [
            'region' => [
                ...
                'deleted_description' => $region->deleted_description,
                'creator' => $region->creator ? [
                    'id'    => $region->creator->id,
                    'name'  => $region->creator->name,
                    'email' => $region->creator->email,
                ] : null,
                'deleter' => $region->deleter ? [
                    'id'    => $region->deleter->id,
                    'name'  => $region->deleter->name,
                    'email' => $region->deleter->email,
                ] : null,
            ],
            'activity' => $activity,
        ]);
    ...
    public function trash(Request $request)
    {
        ...
        $regions = Region::onlyTrashed()
            ...
            ->select('regions.id', 'regions.slug', 'regions.name', 'regions.is_active', 'regions.deleted_at', 'regions.deleted_by', 'regions.deleted_description', 'regions.created_at')
            ->filter($request)
            ->paginate($perPage)
            ->withQueryString();

        return inertia('Regions/Trash', [
            'regions' => $regions,
            'filters' => [
    ...
    public function restore(Request $request, string $slug, RegionService $service)
    {
        abort_unless($request->user()?->hasRole('super'), 403);

        $region = Region::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $service->restore($region);

        return redirect()
            ->route('system_management.regions.trash')
            ->with('success', __('global.restored_success'));
    }

    public function bulkRestore(BulkRestoreRequest $request, RegionService $service)
    {
     ...
     */
    public function forceDelete(ForceDeleteRequest $request, string $slug, RegionService $service)
    {
        $region = Region::onlyTrashed()->where('slug', $slug)->firstOrFail();
        $data   = $request->validated();

        if (trim($data['name_confirmation']) !== $region->name) {
            return back()->withErrors([
                'name_confirmation' => __('global.force_delete_name_mismatch'),
            ]);
        }

        $service->forceDelete($region, $data['reason']);
    ...
    public function deleteSave(DeleteRequest $request, Region $region, RegionService $service)
    {
        ...
        $this->storeUndoableDelete([$region->id]);

        return redirect()
            ->route('system_management.regions.index')
            ->with('success', __('global.deleted_success'))
            ->with('recentDelete', $this->buildRecentDeletePayload([$region->id]));
    }
     ...
     */
    public function duplicate(Request $request, Region $region, RegionService $service)
    {
        $clone = $service->duplicate($region);
    ...
    public function bulkDelete(BulkDeleteRequest $request, RegionService $service)
    {
        $data = $request->validated();
     ...
     */
    public function undoLastDelete(Request $request, RegionService $service)
    {
        $claim = session('regions.recent_delete');
        if (!$claim || !is_array($claim) || empty($claim['ids']) || empty($claim['expires_at'])) {
            return back()->with('error', __('global.undo_failed'));
        }
        if (now()->isAfter($claim['expires_at'])) {
            session()->forget('regions.recent_delete');
            return back()->with('error', __('global.undo_failed'));
        }
    ...
    {
        session(['regions.recent_delete' => [
            'ids'        => array_values($ids),
            'expires_at' => now()->addSeconds((int) config('regions.undo_window_seconds', 60)),
        ]]);
    }
    ...
    protected function buildRecentDeletePayload(array $ids): array
    {
        return [
            'count'   => count($ids),
            'seconds' => (int) config('regions.undo_window_seconds', 60),
        ];
    }

    public function bulkSetActive(BulkSetActiveRequest $request, RegionService $service)
    {
        $data   = $request->validated();
    ...
    public function editAll(Request $request)
    {
        ...
        return inertia('Regions/EditAll', [
            'regions' => $regions,
            'filters' => [
                'name'      => $request->get('name', ''),
                'is_active' => $request->has('is_active') && $request->is_active !== ''
     ...
     */
    public function editAllUpdate(EditAllUpdateRequest $request, RegionService $service)
    {
        ...
        return redirect()
            ->route('system_management.regions.edit_all')
    ...
    protected function buildExportOptions(Request $request, string $format): array
    {
        ...
        return [
            ...
            'title'                   => $data['title']                   ?? __('regions.export_title'),
    ...
    public function exportPdf(Request $request, RegionService $service)
    {
        return $this->dispatchExport($request, $service, 'pdf', \App\Jobs\SystemManagement\Regions\GenerateRegionsPdfJob::class);
    }

    public function exportExcel(Request $request, RegionService $service)
    {
        return $this->dispatchExport($request, $service, 'excel', \App\Jobs\SystemManagement\Regions\GenerateRegionsExcelJob::class);
    }

    public function exportCsv(Request $request, RegionService $service)
    {
        return $this->dispatchExport($request, $service, 'csv', \App\Jobs\SystemManagement\Regions\GenerateRegionsCsvJob::class);
    }

    public function exportWord(Request $request, RegionService $service)
    {
        return $this->dispatchExport($request, $service, 'word', \App\Jobs\SystemManagement\Regions\GenerateRegionsWordJob::class);
    }
     ...
     */
    protected function dispatchExport(Request $request, RegionService $service, string $format, string $jobClass): \Illuminate\Http\RedirectResponse
    {
     ...
     */
    protected function assertExportLimit(RegionService $service, string $format, array $options): void
    {
        ...
        $limit = \App\Models\Setting::getExportLimit('regions', $format);
        if ($limit === 0) return;  // CSV streaming, sin límite

        $count = $service->countForExport($options);
        if ($count > $limit) {
            abort(422, __('regions.export_limit_exceeded', [
    ...
    public function importTemplate()
    {
        return \Maatwebsite\Excel\Facades\Excel::download(
            new \App\Exports\SystemManagement\Regions\RegionsImportTemplate(),
            __('regions.import_template_filename')
        );
    }

    public function import(ImportRequest $request, RegionService $service)
    {
        $data   = $request->validated();
