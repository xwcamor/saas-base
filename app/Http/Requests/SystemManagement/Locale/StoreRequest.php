...
class StoreRequest extends FormRequest
{
    ...
    {
        // Canonicalizar code a lower_UPPER (es_PE). El regex requiere ese formato.
        if ($this->filled('code')) {
            $code = trim($this->code);
    ...
    public function rules(): array
    {
        return [
            'name' => [
                ...
                new UniqueNormalizedName('locales', 'name'),
            ],
            'code' => [
                'required', 'string', 'max:10',
                'regex:/^[a-z]{2}(_[A-Z]{2})?$/',
                Rule::unique('locales', 'code')->whereNull('deleted_at'),
            ],
